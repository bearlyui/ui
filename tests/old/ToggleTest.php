...
class ToggleTest extends TestCase
{
    ...
    {
        $this->blade('<ui:toggle />')->assertSeeHtmlInOrder([
    ...
    {
        $this->blade('<ui:toggle thing="amajig" />')->assertSeeHtmlInOrder([
            '<button',
            'thing="amajig"',
            '>',
        ]);
    ...
    {
        $this->blade('<ui:toggle name="foo" />')->assertSeeHtmlInOrder([
            '<button', 'role="switch"', 'checked:', '>',
    ...
    {
        $this->blade('<ui:toggle wire:model="foo" />')->assertSeeHtmlInOrder([
            '<button', 'role="switch"', 'checked:', '>',
    ...
    {
        $this->blade('<ui:toggle name="foo" value="bar" />')->assertSeeHtml([
    ...
    {
        $this->blade('<ui:toggle :checked="true" />')->assertSeeHtmlInOrder([
            '<button', 'role="switch"', 'checked: true', '>',
        ]);
        $this->blade('<ui:toggle :checked="false" />')->assertSeeHtmlInOrder([
    ...
    {
        $this->blade('<ui:toggle icon-on="foo-icon" icon-off="bar-icon" />')->assertSeeHtmlInOrder([
            '<button', 'role="switch"', 'checked:', '>',
            'foo-icon', 'bar-icon',
        ]);
    }
    ...
    {
        Route::get('/_test', fn () => Blade::render('<ui:toggle name="foo" />'))->middleware('web');
    ...
    public function test_old_input_overrides_value_prop()
    {
        ...
        // Should be checked from old input
        Route::get('/_test', fn () => Blade::render('<ui:toggle name="foo" value="bar" />'))->middleware('web');
        session()->flash('_old_input', ['foo' => true]);
        $this->get('/_test')
            ->assertSeeHtml('name="foo"')
            ->assertSeeHtmlInOrder([
